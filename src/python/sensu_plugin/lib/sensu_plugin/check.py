...
class SensuPluginCheck(SensuPlugin):
    ...
    def output(  # type: ignore[override]  # pylint: disable=too-many-arguments
        ...
        This function is called for every line of output generated by the
        check (but not metrics).

        A pipe is used as a delimiter between the format required by Sensu, and any
        additional metadata that can be used by handlers. e.g the alert key,
        severity, team and alert source.

        There is no return value from this function, as it prints the output
        directly to stdout.

        Args:
            args (list): List of lines to be output by the plugin. Normally this will
            contain a single string
            alert_key (str): Alert key to be used by check handlers
            severity (str): Severity of the alert (Minor, Major, Critical). Used when
            you need more than just WARN and CRIT
            team (str): Team responsible for the alert
            source (str): Source of the alert. If the alert is being generated on a
            proxy host, this should be the hostname of the host that
            generated the alert
        """
        msg = ""
    ...
    def return_final_output(self, rc: int, result_message: str) -> None:
        ...
        The ok, warning, critical functions are dynamically added to the class by the
    ...
    def process_value(  # pylint: disable=too-many-arguments
        ...
        This function does the body of the work for a SensuPluginCheck. It takes the
        current value and compares it against the thresholds defined in the thresholds
        list.

        If the current value is outside of the thresholds, then the appropriate message
        is returned and the appropriate exit code is set.

        Templated messages:

        The templated messages are used to allow the user to define the output for each
        threshold as it is checked. The templated values are surrounded by double
        colons (::) and are replaced with the appropriate values when the message is
        returned.
        ...
        Args:
            alert_id (str | None): The ID of the alert. This is used to identify the
            alert in the thresholds list
            current_value (str | int): The current value to be checked against the
            thresholds
            ok_message (str): The message to be returned if the current value is within
            the thresholds
            alert_message (str): The default templated message to return when the
            threshold is breached. This is used if no warn_message or crit_message
            is passed
            warn_message (str | None): The templated message to return when the warn
            threshold is breached
            crit_message (str | None): The templated message to return when the crit
            threshold is breached
            alert_type (str | None): The type of alert that is being checked. This is
            used in the templated messages
        """
        ...
        self.logger.debug(
            f"Checking thresholds for key: {alert_id} - With current value of"
            f" {current_value}"
        )
        ...
        # Create a CheckResultMetadata object to store the alert messages,
        # alert id and check name
        ...
        for matching_threshold in matching_thresholds:
            ...
            # If result is none, then use threshold_result, otherwise, check the
            # threshold_result rc value, if it's higher, then append the result_messages
            # to the existing result, and set the rc to that value
            if result is None:
    ...
    def _format_messages(
        ...
        if not ok_message:
            raise AttributeError(
                "No ok_message passed. Please ensure correct invocation of"
                " process_value function"
            )
        if not alert_message and not warn_message and not crit_message:
            raise AttributeError(
                "No alert level message passed. Please ensure correct invocation of"
                " process_value function. At least one alert level message is required"
            )
    ...
    def get_thresholds(
        ...
        Returns:
            list[Threshold]: A list of thresholds that match the given alert_id or
            alert_type.
        """
        matching_thresholds = []
        ...
        if alert_id:
            for threshold in self.thresholds:
                ...
                # If we haven't found a specific threshold, but we found the default one
                # save it, in case we need dont find a specific one at all
                if not threshold.id:
    ...
    def process_output_and_rc(
        ...
        Args:
            ...
            result_message_alert (str): The message to return if the threshold is
            breached.
